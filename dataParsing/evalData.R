...
stateSummary <- function(state) {	
	...
	for(i in filenames) {
		...
		return(sort(unique(schoolTable$Zip)));
		#Sets the class of zips to a string
		class(schoolTable$Zip) <- "string";
...
#Builds a data frame of ratios of gender in a zip code are inquired,
#applied, accepted, and enrolled in a certain state.
genderRatioHelper <- function(state)
{
	#Initialize all male variables
	inquiryMaleCounter <- c();
	appliedMaleCounter <- c();
	acceptedMaleCounter <- c();
	enrolledMaleCounter <- c();
	...
	#Initialize all female variables
	inquiryFemaleCounter <- c();
	appliedFemaleCounter <- c();
	acceptedFemaleCounter <- c();
	enrolledFemaleCounter <- c();
	
	inquiryFemaleCount <- c();
	appliedFemaleCount <- c();
	acceptedFemaleCount <- c();
	enrolledFemaleCount <- c();
	...
	for(fileName in filenames) 
	{
		...
		#Reinitialize the counter
    inquiryMaleCount <- NULL;
    appliedMaleCount <- NULL;
    acceptedMaleCount <- NULL;
    enrolledMaleCount <- NULL;
    
   	inquiryFemaleCount <- NULL;
    appliedFemaleCount <- NULL;
    acceptedFemaleCount <- NULL;
    enrolledFemaleCount <- NULL;
		...
		{
		  #########################################################################
			#Find all male inquiries in a zipcode
			inquiryMaleCounter <- subset(schoolTable,	schoolTable$Zip == zip &
														schoolTable$Gender == "M");
			#Append the number of applicants to the list.
			inquiryMaleCount <-c(inquiryMaleCount, nrow(inquiryMaleCounter));
			
			#Find all female inquiries in a zipcode.
			inquiryFemaleCounter <- subset(schoolTable,	schoolTable$Zip == zip &
														schoolTable$Gender == "F");
			#Append the number of applicants to the list.
			inquiryFemaleCount <-c(inquiryFemaleCount, nrow(inquiryFemaleCounter));
			
			#########################################################################
			# Find all male applicants who applied in a zip code
			appliedMaleCounter <- subset(schoolTable,	schoolTable$Zip == zip & 
														schoolTable$Gender == "M" &
														schoolTable$Applied == "Y");
			#Append the number of male applicants to the list.
			appliedMaleCount <-c(appliedMaleCount, nrow(appliedMaleCounter));
			
			# Find all female applicants who applied in a zip code
			appliedFemaleCounter <- subset(schoolTable,	schoolTable$Zip == zip & 
														schoolTable$Gender == "F" &
														schoolTable$Applied == "Y");
			#Append the number of male applicants to the list.
			appliedFemaleCount <-c(appliedFemaleCount, nrow(appliedFemaleCounter));
			
			#########################################################################
			# Find all males who were accepted in a zip code
			acceptedMaleCounter <- subset(schoolTable,	schoolTable$Zip == zip & 
														schoolTable$Gender == "M" & 
														schoolTable$Accepted == "A");
			# Append the number of applicants to the list.
			acceptedMaleCount <-c(acceptedMaleCount, nrow(acceptedMaleCounter));
			
			# Find all females who were accepted in a zip code
			acceptedFemaleCounter <- subset(schoolTable,	schoolTable$Zip == zip & 
															schoolTable$Gender == "F" & 
															schoolTable$Accepted == "A");
			# Append the number of applicants to the list.
			acceptedFemaleCount <-c(acceptedFemaleCount, nrow(acceptedFemaleCounter));
      	
			#########################################################################
			# Find all males who enrolled in a zip code.
			enrolledMaleCounter <- subset(schoolTable,	schoolTable$Zip == zip & 
														schoolTable$Gender == "M" &
														schoolTable$Enrolled == "Y");
			# Append the number of applicants to the list.
			enrolledMaleCount <-c(enrolledMaleCount, nrow(enrolledMaleCounter));
			
			# Find all females who enrolled in a zip code.
			enrolledFemaleCounter <- subset(schoolTable,	schoolTable$Zip == zip & 
														schoolTable$Gender == "M" &
														schoolTable$Enrolled == "Y");
			# Append the number of applicants to the list.
			enrolledFemaleCount <-c(enrolledFemaleCount, nrow(enrolledFemaleCounter));
			#Clears the counters for the next zipcode
			
			inquiryMaleCounter <- NULL;
			appliedMaleCounter <- NULL;
			acceptedMaleCounter <- NULL;
			enrolledMaleCounter <- NULL;
			
			inquiryFemaleCounter <- NULL;
			appliedFemaleCounter <- NULL;
			acceptedFemaleCounter <- NULL;
			enrolledFemaleCounter <- NULL;
		}
		
		#Finds the total population
		inquiryTotalCount <- inquiryMaleCount + inquiryFemaleCount;
		appliedTotalCount <- appliedMaleCount + appliedFemaleCount;
		acceptedTotalCount <- acceptedMaleCount + acceptedFemaleCount;
		enrolledTotalCount <- enrolledMaleCount + enrolledFemaleCount;
		
		#Gets the total percentages
		inquiryMalePercentage <- inquiryMaleCount/inquiryTotalCount;
		inquiryFemalePercentage <- inquiryFemaleCount/inquiryTotalCount;
		
		appliedMalePercentage <- appliedMaleCount/appliedTotalCount;
		appliedFemalePercentage <- appliedFemaleCount/appliedTotalCount;
		
		acceptedMalePercentage <- acceptedMaleCount/acceptedTotalCount;
		acceptedFemalePercentage <- acceptedFemaleCount/acceptedTotalCount;
		
		enrolledMalePercentage <- enrolledMaleCount/enrolledTotalCount;
		enrolledFemalePercentage <- enrolledFemaleCount/enrolledTotalCount;
		
		#Turn data into a dataFrame
		genderSummary <- (data.frame(	Zipcodes = zips,
										InquiryMale = inquiryMaleCount,
										InquiryFemale = inquiryFemaleCount,
										InquiryTotal = inquiryTotalCount,
										InquiryMalePercentage = inquiryMalePercentage,
										InquiryFemalePercentage = inquiryFemalePercentage,
		                              
										AppliedMale = appliedMaleCount,
										AppliedFemale = appliedFemaleCount,
										AppliedTotal = appliedTotalCount,
										AppliedMalePercentage = appliedFemalePercentage,
										AppliedFemalePercentage = appliedMalePercentage,
		                              
										AcceptedMale = acceptedMaleCount,
										AcceptedFemale = acceptedFemaleCount,
										AcceptedTotal = acceptedTotalCount,
										AcceptedMalePercentage = acceptedMalePercentage,
										AcceptedFemalePercentage = acceptedFemalePercentage,
		                              
										EnrolledMale = enrolledMaleCount,
										EnrolledFemale = enrolledFemaleCount,
										EnrolledTotal = enrolledTotalCount,
										EnrolledMalePercentage = enrolledMalePercentage,
										EnrolledFemalePercentage = enrolledFemalePercentage));
		...
		fileName <- paste(path, "genderPercentage", year, state, ".csv", sep=""); 
		write.csv(genderSummary, fileName);
...
nationalLevelWrapper <- function() {
	...
	#Gets the gender ratio information
	genderRatioNationalHelper()
...
#Builds a data frame of ratios of gender in a zip code are inquired,
#applied, accepted, and enrolled in a certain state.
genderRatioNationalHelper <- function()
{
	#Initialize all male variables
	inquiryMaleCounter <- c();
	appliedMaleCounter <- c();
	...
	for(fileName in filenames) 
	{
		...
		fileName <- paste(path, "genderPercentage", year, "US", ".csv", sep=""); 
		write.csv(genderSummary, fileName);
